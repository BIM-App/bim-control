<template>
  <div class="bimBox">
    <div style="padding: 0 0 5px 0; display: flex">
      ...
      <el-button size="small" v-if="showType != 4" @click="toLaunchFull"
        >全屏展示</el-button
      >
      <div v-if="showType == 1 || showType == 3">
        ...
        <el-input
          ...
          @input="margChange($event, 1)"
          style="width: 90px"
          size="small"
          type="number"
          placeholder="请输入左边距范围"
        ></el-input>
        <label for>上</label>
        <el-input
          v-model="topBottom"
          ...
          onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))"
          :max="topMax"
          @input="margChange($event, 2)"
          style="width: 90px"
          size="small"
          type="number"
          placeholder="请输入上边距范围"
        ></el-input>
        <label for>右</label>
        <el-input
          v-model="right"
          ...
          :max="rightMax"
          onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))"
          @input="margChange($event, 3)"
          style="width: 90px"
          size="small"
          type="number"
          placeholder="请输入右边距范围"
        ></el-input>
        <label for>下</label>
        <el-input
          v-model="bottom"
          ...
          :max="bottomMax"
          onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))"
          @input="margChange($event, 4)"
          style="width: 90px"
          size="small"
          type="number"
          placeholder="请输入下边距范围"
        ></el-input>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    margChange(e, s) {
      ...
      this.newData.ftdList[this.ftdUrlIndex].x =
        this.leftRight == 0 ? 0 : this.leftRight / 100
      this.newData.ftdList[this.ftdUrlIndex].y =
        this.topBottom == 0 ? 0 : this.topBottom / 100
      this.newData.ftdList[this.ftdUrlIndex].rightX =
        this.right == 0 ? 0 : this.right / 100
      this.newData.ftdList[this.ftdUrlIndex].bottomY =
        this.bottom == 0 ? 0 : this.bottom / 100
      this.moveMakerBox()
    },
    //返回
    backLefts() {
    ...
    exportModalData() {
      if (this.hintFlag) {
        ...
        function getAllDbIds(viewer) {
          let instanceTree = viewer.model.getData().instanceTree.nodeAccess.dbIdToIndex
    ...
    moveMakerBox() {
      ...
      document.getElementById('markerlayer').style.width = _this.newWidth + 'px'
      document.getElementById('markerlayer').style.height = _this.newHeight + 'px'
    },
    back() {
      this.$router.go(-1)
    },
    init(fUrl, sUrl) {
      ...
      Autodesk.Viewing.Initializer(options, function () {
        ...
        _this.viewer3D = new Autodesk.Viewing.Private.GuiViewer3D(viewer3DContainer, {})
        viewer3D = _this.viewer3D

        var viewer2DContainer = document.getElementById('viewer2D')
        _this.viewer2D = new Autodesk.Viewing.Private.GuiViewer3D(viewer2DContainer, {})
      ...
      function initializeMarker() {
        ...
        viewer2D.addEventListener(Autodesk.Viewing.CAMERA_CHANGE_EVENT, function (event) {
        ...
        viewer3D.addEventListener(Autodesk.Viewing.CAMERA_CHANGE_EVENT, function (event) {
      ...
      function updateMarkerToCamera() {
        ...
        document.getElementById('marker').style.left = markX3D * _this.newWidth + 'px'
      ...
      function updateCameraToMarker(skipPosition, skipTarget) {
        ...
        if (!skipTarget) {
          ...
          viewer3D.navigation.setTarget(
            positionToVector3([position.x + direction[0], position.y + direction[1], 6])
          )
        }
      }
      ...
      function Marker(container, customMarker) {
        ...
        var arrow =
          customMarker || document.createElementNS('http://www.w3.org/2000/svg', 'image')
        if (!customMarker) {
        ...
        function rotateMarker(angle, unit) {
          ...
          svg.style.MozTransform = 'translate(-10px, -10px)  rotate(' + angle + unit + ')'
          svg.style.msTransform = 'translate(-10px, -10px) rotate(' + angle + unit + ')'
          svg.style.OTransform = 'translate(-10px, -10px)  rotate(' + angle + unit + ')'
          svg.style.transform = 'translate(-10px, -10px)  rotate(' + angle + unit + ')'
        }
        ...
        this.alignMarkerDirection = function () {
          var angleToxAxis = directionMap.angleBetween(directionMap.currentDirection, [
            1,
            0,
          ])
      ...
      function worldToPaper(position) {
        ...
        var data1 = {
          x: ((position.x + Math.abs(num2.min.x)) / viewer3D_data.x) * viewer2D_data.x,
          y: ((position.y + Math.abs(num2.min.y)) / viewer3D_data.y) * viewer2D_data.y,
          z: num2.min.z + 2,
        }
  ...
  mounted() {
    ...
    if (this.svfUrl.length != 0 && this.ftdUrl.length != 0) {
      ...
      this.init(this.ftdUrl[this.ftdUrlIndex].path, this.svfUrl[this.svfUrlIndex].path)
    }
    if (this.svfUrl.length != 0 && this.ftdUrl.length == 0) {
      this.svfUrlIndex = 0
      // this.viewer33D = true;
