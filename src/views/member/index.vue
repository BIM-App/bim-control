...
import { addMembersData, deleteMemberByID, updateMemberByID } from '../../api/member'
export default {
  data() {
    return {
      memberDialogVisible: false,
  ...
  methods: {
    handleEdit(index, row) {
      ...
      this.$prompt('编辑成员角色', '提示', {
        ...
        updateMemberByID(data).then((res) => {
          console.log(res)
          if (res.data.status === 200) {
            // 调用根据PID查询项目下所有成员列表接口
            findProjectMembersByPID(getProjectPID()).then((res) => {
              console.log(res)
              setMember(res.data)
    ...
    deleteMember(index, row) {
      const _this = this
      // console.log(index, row)
      deleteMemberByID(row.id).then((res) => {
        console.log(res)
