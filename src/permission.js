...
router.beforeEach(async(to, from, next) => {
  ...
  if (hasRole) {
    if (to.path === '/login') {
      ...
      if (router.options.routes.length === 10 && getRole() === 'super') {
        // super权限
        next()
      } else if (router.options.routes.length === 10 && getRole() === 'admin') {
        // admin权限
        next()
      } else if (router.options.routes.length === 10 && getRole() === 'staff') {
        // staff权限
        next()
      } else {
        try {
